BaumWelch.dthmm <- function (object, control = bwcontrol(), ...){
    x <- object$x
    Pi <- log(object$Pi)
    delta <- log(object$delta)
    if ( sum( delta == -Inf ) > 0 ){
        delta[ delta == -Inf ] <- -1e48
        warning("delta has zero-probability states")
    }
    if ( sum( Pi == -Inf ) > 0 ){
        Pi[ Pi == -Inf ] <- -1e48
        warning("Pi has zero-probability transistions")
    }
    distn <- object$distn
    pm <- object$pm
    tol <- control$tol
    ...
    for (iter in 1:control$maxiter) {
        ...
        tPi<-Pi
        tPi[,]<--Inf
        diag(tPi)<--apply(cond$v, MARGIN = 2, FUN = lsum)
        Pi <- lmatMult(tPi, 
            apply(cond$v, MARGIN = c(2, 3), FUN = lsum))
        delta <- cond$u[1, ]
